...
from api.models import Collaborative, CollaborativeMetadata, Geography, Metadata
...
class CollaborativeMetadataSerializer(serializers.ModelSerializer):
    metadata_item = MetadataSerializer()

    class Meta:
        model = CollaborativeMetadata
        fields = ["metadata_item", "value"]

    def to_representation(self, instance: CollaborativeMetadata) -> Dict[str, Any]:
...
class CollaborativeDocumentSerializer(serializers.ModelSerializer):
    metadata = CollaborativeMetadataSerializer(many=True)
    tags = serializers.ListField()
    sectors = serializers.ListField(default=[])
    geographies = serializers.ListField(default=[])
    slug = serializers.CharField()
    is_individual_collaborative = serializers.BooleanField()
    ...
    class UseCaseSerializer(serializers.Serializer):
        title = serializers.CharField()
...
class SearchCollaborative(PaginatedElasticSearchAPIView):
    serializer_class = CollaborativeDocumentSerializer
    document_class = CollaborativeDocument
    ...
    def add_aggregations(self, search: Search) -> Search:
        ...
        for aggregation_field in self.aggregations:
            if aggregation_field.startswith("metadata."):
                aggregate_fields.append(aggregation_field.split(".")[1])
            else:
                search.aggs.bucket(
    ...
    @trace_method(name="generate_q_expression", attributes={"component": "search_collaborative"})
    def generate_q_expression(self, query: str) -> Optional[Union[ESQuery, List[ESQuery]]]:
        if query:
            ...
            for field in self.searchable_fields:
                ...
                elif field.startswith("use_cases."):
                    queries.append(
                        ESQ(
                            "nested",
                            path="use_cases",
    ...
    def add_filters(self, filters: Dict[str, str], search: Search) -> Search:
        ...
        for filter in filters:
            ...
            elif filter in ["status", "is_individual_collaborative"]:
                search = search.filter("term", **{filter: filters[filter]})
            ...
            elif filter in ["datasets.slug", "use_cases.slug"]:
                path = filter.split(".")[0]
                search = search.filter(
                    "nested",
                    path=path,
    ...
    @trace_method(name="add_sort", attributes={"component": "search_collaborative"})
